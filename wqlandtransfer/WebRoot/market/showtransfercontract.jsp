...
<div class="main">
  ...
  <tbody>
  		合同书<p>
  		${sessionScope.buyer }<br>
  		${sessionScope.saler }<br>
  		${sessionScope.land.id }
       <tr>
      <td height="60" colspan="6" align="center">
      	<c:choose>
      		<c:when test="${sessionScope.land.status == 4}">
      			<a href="openTransferContractAction.action"><input type="submit" value="生成合同书"></a>
      		</c:when>
      		<c:when test="${sessionScope.land.status == 5}">
      			<a href="confirmTransferContractAction.action"><input type="submit" value="确认合同书"></a>
      		</c:when>
      	</c:choose>
      </td>
      </tr>
