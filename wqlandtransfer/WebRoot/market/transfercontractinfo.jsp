	...
	<div class="content_box">
		...
		<TR>
			...
			<table width="800" cellspacing="0" cellpadding="0">
				...
				<c:forEach var="land" items="${sessionScope.contractLands}"
					varStatus="s">
					<tr>
						<td>
							${s.count }
						...
						<td>
							<c:choose>
								<c:when test="${land.status == 4}">
									<a href="showTransferContractAction.action?index=${s.index }">生成合同</a>
								</c:when>
								<c:when test="${land.status == 5}">
									<a href="showTransferContractAction.action?index=${s.index }">查看合同</a>
								</c:when>
								<c:when test="${land.status == 6}">
									合同已生成
								</c:when>
