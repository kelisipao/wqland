		...
		<div class="body">
			...
			<div id="bodyshow">
				...
				<div class="menubody">
					<p>
					<div id="apply">
						<c:forEach var="pic" items="${sessionScope.declare.allPics}" varStatus="s" begin="1">
						<td><img src="../${pic }" width="50" height="50" />			
						${fn:substring(pic,"25",fn:indexOf(pic,'.')) }</td>
						</c:forEach>
						<p>
						<c:choose>
							<c:when
								test="${sessionScope.declare.status == 0 || sessionScope.declare.status == 1}">
								<a href="declarationPassAction.action?role=0"><input
										type="submit" value="通过" /> </a>
								<a href="declarationRejectAction.action?role=0"><input
										type="submit" value="驳回" />
								</a>
							</c:when>
						</c:choose>
					</div>
